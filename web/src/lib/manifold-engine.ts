/**
 * Manifold WASM Engine for Helscoop
 *
 * Uses manifold-3d (the official WASM build of the same Manifold CSG kernel
 * the C++ viewer uses) to compute geometry. This ensures web rendering
 * matches the C++ viewer exactly.
 *
 * The coordinate system is Z-up (matching the C++ viewer).
 */
...
export interface TessellatedObject {
...
function tessellateManifold(cm: ColoredManifold): TessellatedObject | null {
...
export interface EvaluateOptions {
  onProgress?: (meshes: TessellatedObject[], done: number, total: number) => void;
}

export interface ManifoldSceneResult {
...
export async function evaluateScene(script: string, options?: EvaluateOptions): Promise<ManifoldSceneResult> {
  ...
  // Transform functions — these return new Manifolds (immutable API)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  // Wall primitive (matching C++ viewer's wall implementation)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  // Preprocess script: strip `export` keywords
  ...
  try {
    ...
    // scene.add() proxy for the simple API
    const sceneProxy = {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
`;
    ...
    // Sort manifolds by triangle count — cheap ones first for progressive rendering
    ...
    const meshes: TessellatedObject[] = [];
